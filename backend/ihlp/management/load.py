...
class Load:
    ...
    def load(self):
        ...
        PATH = BASE_DIR + '/management/commands/load/'
        ...
        Request.objects.using('ihlp').filter(id__in=list(requests.id.values)).delete()
        ...
        Item.objects.using('ihlp').filter(id__in=list(items.id.values)).delete()
        bulk_mgr = BulkCreateManager(chunk_size=50)
        ...
        Relation.objects.using('ihlp').filter(id__in=list(relations.id.values)).delete()
        bulk_mgr = BulkCreateManager(chunk_size=50)
        ...
        Object.objects.using('ihlp').filter(id__in=list(objects.id.values)).delete()
        bulk_mgr = BulkCreateManager(chunk_size=50)
