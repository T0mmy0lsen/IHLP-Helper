...
df_user = pd.read_csv('data/label_responsible.csv')
...
df['label_responsible_time_encoded'] = df.apply(lambda x: int(x['tmp']), axis=1)

df[['id', 'label_responsible', 'label_responsible_time_encoded']].to_csv('data/label_time_encoded.csv', index=False)