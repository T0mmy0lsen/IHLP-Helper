...
for index, row in unique_combinations.iterrows():
    ...
    mean_data = filtered_data.groupby(['model', 'algorithm'])[average_columns].mean().reset_index()
    mean_data = mean_data.round(1)
    mean_data.columns = ['model', 'algorithm'] + [f"{col}" for col in mean_data.columns if col not in ['model', 'algorithm']]
...
total_scores = pd.concat(all_mean_data, axis=0).groupby(['model', 'algorithm'])['score'].sum().reset_index()

# Reshape the DataFrame
pivot_total_scores = total_scores.pivot_table(index='model', columns='algorithm', values='score', fill_value=0)

# Convert index back to column
pivot_total_scores.reset_index(inplace=True)

dataframe_to_image(pivot_total_scores, "Total Scores [model, algorithm]")
