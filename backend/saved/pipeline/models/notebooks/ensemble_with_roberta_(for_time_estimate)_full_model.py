...
output_time = tf.keras.layers.Dense(units=100, activation='softmax', name='responsible')(output_time)
...
class CustomAccuracy(tf.keras.losses.Loss):
    ...
    def call(self, t, p):
        ...
        return tf.reduce_mean(tf.math.square(tf.math.pow(p * tf.math.log(t + 1), 2)), axis=-1)
...
# validation = np.concatenate([y['responsible'] for x, y in V])
