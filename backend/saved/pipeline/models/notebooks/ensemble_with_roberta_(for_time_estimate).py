...
from transformers import AutoTokenizer, TFXLMRobertaForSequenceClassification, create_optimizer
...
def sample(
    ...
    for e in list(range(100)):
        ...
        if len(indices) < sample_size:
            for _ in list(range(sample_size - len(indices))):
            ...
            index_to_remove = index_to_remove + indices[-(len(indices) - sample_size):]
...
def data(batch_size=batch):
    ...
    arr_feature_vector = pd.read_csv('cache/arr_feature_vector.csv', usecols=['2', '3', '4']).values
    ...
    train_0_feature_vector, \
    train_1_feature_vector, \
    train_0_received_by, \
    train_1_received_by, \
    train_0_title, \
    train_1_title, \
    ...
    train_feature_vector = train_0_feature_vector + train_1_feature_vector
    train_received_by = train_0_received_by + train_1_received_by
    train_title = train_0_title + train_1_title
    train_omk = train_0_omk + train_1_omk
    # train_texts = train_0_texts + train_1_texts
    # train_time = train_0_time + train_1_time
    train_responsible = train_0_responsible + train_1_responsible
...
lengths = [len_feature_vector, len_feature_received_by, len_feature_title, len_feature_omk]
learning_rates = [0.000316, 0.00158, 0.00025, 0.00031]
learning_rates = [0.000250, 0.00125, 0.00020, 0.00025]
learning_rates = [0.7, 0.00125, 0.00020, 0.00025]
...
for i, el in enumerate([
    ...
    layer_input = tf.keras.layers.Input(shape=(lengths[i],), dtype=tf.float64, name=el)
    ...
    layer = tf.keras.layers.Dropout(.1)(layer)
    layer = tf.keras.layers.Dense(
        units=lengths[i],
        activation='softmax',
        kernel_initializer='random_normal',
    ...
    layer = tf.keras.layers.Dropout(.1)(layer)
    output = tf.keras.layers.Dense(units=100, activation='softmax', name='time')(layer)
    ...
    optimizer, _ = create_optimizer(init_lr=learning_rates[i], num_warmup_steps=10000, num_train_steps=total_train_steps)

    model.compile(
        loss=tf.keras.losses.CategoricalCrossentropy(from_logits=False),
        optimizer=tf.keras.optimizers.SGD(learning_rate=learning_rates[i]),
        # optimizer=optimizer,
        metrics=['accuracy'],
    )
    ...
    model.fit(
        ...
        validation_data=V,
        callbacks=[
            CSVLogger(f'logs/{el}.csv', append=True, separator=';'),
            # tf.keras.callbacks.EarlyStopping(monitor='loss', patience=4),
            # tf.keras.callbacks.LearningRateScheduler(lambda epoch: 1e-6 * 10 ** (((epoch - 1 - epochs) * (-1)) / 10)),
        ]
    )

    version = 'colab_21_11_1531'
    version_save = el
