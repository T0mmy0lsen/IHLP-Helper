...
class WordEmbedding:
    ...
    def __init__(self, table='request_tasktype_simple', index_text='text', dim=64):
        ...
        path = f'{config.BASE_PATH}/output/word_embedding/{table}_{dim}.txt'
        if os.path.exists(path):
            msg = f'Can not override existing word embedding {path}'
            print(msg)
            return

        file_path = f'{config.BASE_PATH}/model/{table}_word_embedding.csv'

        if not os.path.exists(file_path):
            df = pd.read_csv(f'{config.BASE_PATH}/model/tmp/word_embedding/{table}.csv')
            df = df.fillna('')
            df = df[[index_text]]
    ...
    def write_out(self, targets, contexts, labels, idx):
        ...
        if len(str_data) > 0:
            f = open(f'{config.BASE_PATH}/model/tmp/word_embedding/targets_{idx}.dat', 'a')
            f.write(str_data)
            f.close()
        ...
        if len(str_data) > 0:
            f = open(f'{config.BASE_PATH}/model/tmp/word_embedding/contexts_{idx}.dat', 'a')
            f.write(str_data)
            f.close()
        ...
        str_data = "\n".join([", ".join([str(y) for y in x]) for x in tmp])
        if len(str_data) > 0:
            f = open(f'{config.BASE_PATH}/model/tmp/word_embedding/labels_{idx}.dat', 'a')
            f.write(str_data)
            f.close()
